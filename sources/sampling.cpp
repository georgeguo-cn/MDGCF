...
py::array_t<int> sample_mi_negatives(int user_num, int neg_num)
{
    py::array_t<int> S_array = py::array_t<int>({neg_num, user_num});
    ...
    for (int j = 0; j < neg_num; j++)
    {
        for (int u = 0; u < user_num; u++)
        {
            ...
            do
            {
                negitem = randint_(user_num);
            } while (negitem != u);
            ptr[j * user_num + u] = negitem;
        }
    }
    return S_array;
}
...
PYBIND11_MODULE(sampling, m)
{
    ...
    m.def("sample_mi_negatives", &sample_mi_negatives, "sampling negatives for mi",
          "user_num"_a, "neg_num"_a);
}
